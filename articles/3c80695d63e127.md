...
これは `github.com/kataras/iris/v12/mvc` パッケージの `Configure()` メソッドを使用しています。`mvc.Configure()` は Iris のMVCパターンを利用するためのメソッドで、 特定のパスグループに対してコントローラやロジックをマッピングすること及び依存性の注入に使用されます。
...
## controller/users/users_controller.go（ポイント②）
...
[基本編](https://zenn.dev/mkosakana/articles/3c80695d63e127#iris-を使ってみる（基本編）) で書いたコードのように、`app.Get("GET", "/users/list", 処理...)`、`app.Get("GET", "/users/details", 処理...)` というように、URLとそれに紐づける処理を”ベタ書き”した関数をたくさん作っていくのは大変です。
かといって「/users」のような同じパスにのみにしか GET/POST/PUT/DELETE... 毎の処理を作れないとなると拡張性が低いです。

**Iris（のMVCサポート機能）では、URLルートをコントローラのアクションにマッピングする機能が提供されています。**
具体的には、**コントローラーのメソッドの名前を変えることによって、同じパスグループ内での細かいパスの違いに対応できます。**

例えば `main.go` などでURLのグルーピングとコントローラーの紐付けをしたのち（今回は「/users」）、GET で `/users/list` というURLを受け取る際の処理は、紐付けられたコントローラーに `GetList()` というメソッドを、
POST で `/users/details` というURLを受け取る際の処理は、コントローラーに `PostDetails()` というメソッドを作りその中に処理を書きます。

こうすることで、**受け取る想定のURLの、`app.Party()` などで最初にグループ化指定された箇所（今回は「/users」）以降のパス名を名前に持つコントローラーのメソッドに Iris が自動でアクションをマッピングします。**

まとめると、例えば `main.go` などでURLのグルーピングとコントローラーの紐付けをしたのち（今回は「/users」）、
- GET で `/users/list` というURLでリクエストがきた場合、Iris（のMVCサポート機能）が自動でそれを判別してコントローラーの `GetList()` という名前のメソッドの処理が行われる
- POST で `/users/details` というURLでリクエストがきた場合、Iris（のMVCサポート機能）が自動でそれを判別してコントローラーの `PostDetails()` という名前のメソッドの処理が行われる
 
となります。

:::message
さらにここでコントローラ内の `PutDetailsBy(id int)`、`DeleteDetailsBy(id int)` にもご注目ください。
今回はパスパラメーターで `id` が送られて来ますが、（/users/details/{id} など）
